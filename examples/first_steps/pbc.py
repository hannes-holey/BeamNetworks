...
if __name__ == "__main__":
    ...
    sm, color = _array_to_colors(np.vstack([ty, tx, txy]), cmap='coolwarm')
    plt.colorbar(sm, ax=ax[1, :], orientation='vertical',
                 label='Rotation (edge avg.)')

    sm, color = _array_to_colors(np.hstack([py._sVM, py._sVM, pxy._sVM]), cmap='plasma')
